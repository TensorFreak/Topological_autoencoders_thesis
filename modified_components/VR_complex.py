# modified Vietoris--Rips complex with minimal persistence
...
class VietorisRipsComplex(nn.Module):
    ...
    def __init__(
        ...
        dim=1,
        p=2,
        threshold=numpy.inf, # actually this is max_persistence
        min_persistence=None,  # new parameter
    ...
    def _extract_generators_and_diagrams(
        ...
        if dim0:
            ...
            if self.min_persistence is not None:
                ...
                persistence = destroyers - creators
                mask = persistence > self.min_persistence                
                # Filter features with sufficient persistence
                persistence_diagram = persistence_diagram[mask] 
                gens = gens[mask.cpu().numpy()]  # Apply same filter to generators
